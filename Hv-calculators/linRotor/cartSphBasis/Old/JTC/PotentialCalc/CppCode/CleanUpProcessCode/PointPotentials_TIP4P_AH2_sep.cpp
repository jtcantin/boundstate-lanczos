...
int AlaviMapH2GridToHGrid(VECT CM, double theta, double phi, double *pointGrid_max, double *pointGrid_d_i, int *pointGrid_num, double H_CM_dist, int numDim) {
	int j, *indices, displacement;
	double c_i, c_i_min, d_c, e_i, numerator;
	
	indices = new int [numDim];
	...
	for (j=0; j<numDim; j++) {
		...
		c_i_min = (-1.0/2.0)*pointGrid_max[j];
		...
		else if (indices[j]>=pointGrid_num[j]) {
			indices[j] = pointGrid_num[j]-1;
		}
	}
	//Get displacement in a 1D storage array corresponding to an (x, y, z, etc.) index in a numDim system
	displacement = disp(indices, numDim, pointGrid_num);
...
#pragma omp for schedule(dynamic, chunk)
	for (i=0; i<systemSize; i++) {
		...
		index = AlaviMapH2GridToHGrid(universe_grid[i], theta, phi, box_i_max, d_i, box_i_num, H2_H1_Z, dim_num);
...
#pragma omp for schedule(dynamic, chunk)
	for (i=0; i<systemSize; i++) {
		//Get which point in the point potential corresponds to H1 of the H2 molecule at universe_grid[i] and theta and phi
		index = AlaviMapH2GridToHGrid(universe_grid[i], theta, phi, box_i_max, d_i, box_i_num, H2_H2_Z, dim_num);
