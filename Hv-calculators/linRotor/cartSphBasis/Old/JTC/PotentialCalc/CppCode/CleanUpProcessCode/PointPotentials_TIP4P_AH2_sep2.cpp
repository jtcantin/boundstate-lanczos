...
void Alavi_H2_Eng(double *H2_pot, double *CM_pot, double *H_pot, H2_orient H2_mol, universeProp universe) {
...
#pragma omp for schedule(dynamic, chunk)
	for (i=0; i<systemSize; i++) {
		...
		//Get which point in the point potential corresponds to H1 of the H2 molecule at universe_grid[i] and theta and phi
		index = AlaviMapH2GridToHGrid(&H2_mol_orient, &H_universe, H2_H2_Z);
...
#pragma omp for schedule(dynamic, chunk)  //Possibly try: http://software.intel.com/en-us/articles/openmp-loop-collapse-directive
	...
	*/
	...
	universeProp H_universe;
	
	H_universe.grid = universe_grid;
	H_universe.grid_max = box_i_max;
	H_universe.d_i = d_i;
	H_universe.grid_num = box_i_num;
	H_universe.numDim = dim_num;
	H_universe.sysSize = systemSize;
