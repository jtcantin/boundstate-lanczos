...
/* Original class made by Dr. Pierre-Nicholas Roy
 Modified by Joshua Tyler Cantin (JTC) as discussed in matvec2.h
 */
...
{
  delete[] TheMatrix;
  delete[] b; //Added by JTC to free this memory; the dereferenced value of b[0] holds TheMatrix and the rest of b needs to be deleted.
}
...
//The following code written by Joshua Cantin
vector::vector () {
	Sz=1;
	TheVector= new double[Sz];
	
	if (!TheVector) {
		cerr << "Vector not allocated." << endl;
		exit(1); // allocation test
	}
	
	TheVector[0] = 0.;
}

void vector::setCoor(double* new_coor){
	int i;
...
void vector::setSz(int dim){
	int i;
	Sz = dim;
	delete [] TheVector; //Need to dealocate the old memory, or there is a memory leak
	TheVector = new double [Sz];
...
double vector::eNorm () const {
	...
	for (i=0; i<Sz; i++) {
		sumSqr += TheVector[i] * TheVector[i];
	}
	return sqrt(sumSqr);
}